...
func recvBitfield(conn net.Conn) (bitfield.Bitfield, error) {
	...
	if msg.ID != message.MsgBitfield {
		return nil, fmt.Errorf("execpted bitfield %d but got %d", message.MsgBitfield, msg.ID)
	}
...
func New(peer peers.Peer, peerID, infoHash [20]byte) (*Client, error) {
	...
	// bitfield message may only be sent immediately after the handshaking sequence
